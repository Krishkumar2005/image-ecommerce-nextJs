...
import { IMAGE_VARIANTS, IProduct, ImageVariant, ImageVariantType } from '@/models/Product'
import { useSession } from 'next-auth/react'

import { useParams, useRouter } from 'next/navigation'
import { useEffect, useState } from 'react'
import { Loader2, AlertCircle, Check, Image as ImageIcon, HelpingHand } from "lucide-react";
...
export default function ProductPage() {
  ...
  useEffect(() => {
    console.log("Inside useEffect of specific product");
    ...
    const fetchedProduct = async () => {
      ...
      try {
        ...
        console.log("Inside try of specific Product", typeof id);

        const data = await apiClient.getProduct(id.toString());
        
        console.log("After fetch of Specific Product");
        setProduct(data);
        console.log("data ", data);
      } catch (error) {
        console.error("Error fetching product: ", error)
        setError(error instanceof Error ? error.message : "Failed to fetch/load product")
      } finally {
        setLoading(false);
      }
    };
  ...
  const handlePurchase = async (variant: ImageVariant) => {
    ...
    try {
      ...
      console.error("handle purchase ", error);
      showNotification(
  ...
  type cropModeType = "extract" | "pad_resize" | "pad_extract"
    ...
    return [
      {
        ...
        cropMode:"extract" as cropModeType,
        focus: "center",
        quality: 60,
      },
    ];
  ...
  return (
     <div className="container mx-auto px-4 py-8 w-l">
